...
The test environment pins jsdom 30.0.1. With jsdom 30.1.1, Vitest 5.0.1 cannot initialize DOM workers under the exact Bun canary: window error handling fails the `EventTarget.addEventListener` receiver check. Keep the pin until that runtime combination passes the DOM tests.
